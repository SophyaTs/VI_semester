...
public class UserInterface extends JFrame{
	...
	UserInterface(){
		...
		comboCountry.addActionListener (new ActionListener () {
		    public void actionPerformed(ActionEvent e) {
		        ...
		        cCnt =CountryDAO.findByName((String) comboCountry.getSelectedItem());
		        countryMode = true;
		        countryPanel.setVisible(true);
				cityPanel.setVisible(false);
				fillCountryFields();
		...
		comboCity.addActionListener (new ActionListener () {
		    public void actionPerformed(ActionEvent e) {
		        ...
		        cCt = CityDAO.findByName((String)comboCity.getSelectedItem());
		        countryMode = false;
		        countryPanel.setVisible(false);
				cityPanel.setVisible(true);
				fillCityFields();
	...
	private static void delete() {
		if(editMode) {
			if(countryMode) {				
				var list = CityDAO.findByCountryId(cCnt.getId());
				for(City c: list) {
					comboCity.removeItem(c.getName());
					CityDAO.delete(c);
				}
				CountryDAO.delete(cCnt);
	...
	private static void fillCityFields() {
		if(cCt == null)
			return;
		Country cnt = CountryDAO.findById(cCt.getCountryId());
